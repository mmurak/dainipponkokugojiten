...
for filename in  sorted(glob.glob('*.pdf')):
...
pdfDirs = [ 'digidepo_1870620_PDF', 'digidepo_1870644_PDF', 'digidepo_1870670_PDF', 'digidepo_1870700_PDF', 'digidepo_1870727_PDF']
for dirname in pdfDirs:
    if not os.path.exists(f'{resourceDir}{os.sep}{dirname}'):
        exit()
if os.path.exists(f'{resourceDir}{os.sep}dainipponkokugo'):
    exit()
os.makedirs(f'{resourceDir}{os.sep}dainipponkokugo')
shutil.copyfile(f'{originalIdx}{os.sep}dainipponkokugo.js', f'{resourceDir}{os.sep}dainipponkokugo{os.sep}index.js')
with open('config.js', 'r') as inf:
    readData = inf.readlines()
for i, elem in enumerate(readData):
    if elem.strip() == 'let configInfo = [':
        readData.insert(i+1, f'    ["修訂大日本國語辭典（1941）", "dainipponkokugo"],\n')
        break
with open('newconfig.js', 'w') as of:
    for rec in readData:
        of.write(f'{rec}')
